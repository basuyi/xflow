...
public class ApplicationDialog extends TitleAreaDialog {
	...
	public ApplicationDialog(Shell parentShell,
		...
		if (applicationListDialog.activityPropDialog.allApplicationList == null
				|| applicationListDialog.activityPropDialog.allApplicationList
						.size() < 1) {
		...
		if (this.application != null
				&& this.application.getApplicationId().length() > 0) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		tvAppPara = new TableViewer(composite2, SWT.H_SCROLL | SWT.V_SCROLL
		...
		btnAppParaEdit.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				WorkflowParameterDialog paraDialog = new WorkflowParameterDialog(
						...
						(ParameterEntire) (tvAppPara.getElementAt(tvAppPara
	...
	private void setPrioTextContent() {
		...
		if (this.application != null
			...
			this.comboAppType.select(new Integer(application
					.getApplicationType()).intValue());
	...
	protected void buttonPressed(int buttonId) {
		if (buttonId == IDialogConstants.OK_ID) {
			if (this.txtAppName.getText() == null
					&& this.txtAppName.getText().length() < 1) {
				MessageDialog.openConfirm(UIPlugin.getDefault().getWorkbench()
						.getActiveWorkbenchWindow().getShell(), "应用名称不能为空",
						"应用名称不能为空");
				return;
			}
			if (this.comboAppType.getSelectionIndex() < 0) {
				MessageDialog.openConfirm(UIPlugin.getDefault().getWorkbench()
						.getActiveWorkbenchWindow().getShell(), "应用类型不能为空",
						"应用类型不能为空");
				return;
			}
			if (this.application != null
				...
				this.application.setApplicationType((new Integer(
						this.comboAppType.getSelectionIndex())).toString());
			...
			// for(int i =
			// 0;i<this.applicationListDialog.allApplicationList.size();i++)
			// {
			...
			// }
			// }
			if (!this.applicationListDialog.activityPropDialog.allApplicationList
					.contains(this.application)) {
				this.applicationListDialog.activityPropDialog.allApplicationList
