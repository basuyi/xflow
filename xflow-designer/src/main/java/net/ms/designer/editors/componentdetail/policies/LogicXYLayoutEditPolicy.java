...
import net.ms.designer.editors.componentdetail.commands.SetConstraintCommand;
...
public class LogicXYLayoutEditPolicy extends
	...
	//		Request request, KCGField part, Command cmd, boolean horizontal) {
	//	Command result = cmd;
	//	
	...
	protected Command chainGuideDetachmentCommand(Request request,
			Element part, Command cmd, boolean horizontal) {
		Command result = cmd;
...
//		if(getHost().getModel() instanceof Table && part instanceof Table)
//			return null;
////		if((part instanceof Field)&& !(getHost().getModel()instanceof Table) ){
////			return null;
//		}
//		
...
////		add.setChild(part);
////		add.setLabel(Messages.getString("LogicXYLayoutEditPolicy.AddCommand")); //$NON-NLS-1$
////		add.setDebugLabel("LogicXYEP add subpart");//$NON-NLS-1$
//
//		SetConstraintCommand setConstraint = new SetConstraintCommand();
//		setConstraint.setLocation(rect);
//		setConstraint.setPart(part);
//		setConstraint.setLabel(Messages.getString("LogicXYLayoutEditPolicy.ConstraintCommand")); //$NON-NLS-1$
//		setConstraint.setDebugLabel("LogicXYEP setConstraint");//$NON-NLS-1$
//
//		//System.out.println(part + " : rect = " + rect);
//
...
//		return chainGuideDetachmentCommand(request, part, cmd, false);
	...
	protected Command createChangeConstraintCommand(
		...
		Element part = (Element) child.getModel();
		cmd.setPart(part);
	...
	protected EditPolicy createChildEditPolicy(EditPart child) {
		...
		if (child instanceof LabelEditPart
				|| child instanceof CommonEditPart) {
			ResizableEditPolicy policy = new LogicResizableEditPolicy();
	...
	protected IFigure createSizeOnDropFeedback(CreateRequest createRequest) {
		...
		//	else
		if (createRequest.getNewObject() instanceof Label)
...
//			figure.setBackgroundColor(LogicColorConstants.ghostFillColor);
	...
	protected Command getCloneCommand(ChangeBoundsRequest request) {
		CopyCommand clone = new CopyCommand();
		...
		while (i.hasNext()) {
			...
			clone.addPart((Element) currPart.getModel(),
		...
		if (guidePos != null) {
			...
			//clone.setGuide(findGuideAt(guidePos.intValue(), true),
			// hAlignment, true);
		}
		...
		if (guidePos != null) {
			int vAlignment = ((Integer) request.getExtendedData().get(
					SnapToGuides.KEY_VERTICAL_ANCHOR)).intValue();
			//clone.setGuide(findGuideAt(guidePos.intValue(), false),
			// vAlignment, false);
		}
	...
	protected Command getCreateCommand(CreateRequest request) {
		...
		boolean canExc = (model instanceof Container);
//		&& (request.getNewObject() instanceof ChildTable);
		
		// µã»÷¼ÓÈë
		if(!canExc && !(request.getNewObject() instanceof Label) && !(model instanceof Table))
//			&& !(request.getNewObject()instanceof FlowField)
			return null;
		
		CreateCommand create = new CreateCommand();
		create.setParent((Container) getHost().getModel());
		Element newPart = (Element) request.getNewObject();
		create.setChild(newPart);
		Rectangle constraint = (Rectangle) getConstraintFor(request);
		create.setLocation(constraint);
//		create.setLabel(Messages.getString("LogicXYLayoutEditPolicy.CreateCommand")); //$NON-NLS-1$
	...
	protected Command getOrphanChildrenCommand(Request request) {
		...
		List parts = gr.getEditParts();
		CompoundCommand result = new CompoundCommand(Messages.getString("LogicXYLayoutEditPolicy.CompoundCommand")); //$NON-NLS-1$
//		for (int i = 0; i < parts.size(); i++) {
//			OrphanChildCommand orphan = new OrphanChildCommand();
//			orphan.setChild((Element) ((EditPart) parts.get(i)).getModel());
//			orphan.setParent((Container) getHost().getModel());
//			orphan.setLabel(Messages.getString("LogicXYLayoutEditPolicy.OrphanCommand")); //$NON-NLS-1$
//			result.add(orphan);
//		}
